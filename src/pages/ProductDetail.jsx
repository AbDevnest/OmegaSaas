...
export default function ProductDetail() {
  ...
  if (role === "user" && !appState.isPublished(product.id)) {
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-700">
        This product is hidden for user view.
        <div className="mt-3">
          <Link
            to="/products"
            className="inline-flex items-center gap-2 rounded-lg border border-amber-300 px-3 py-1.5 text-sm font-semibold"
          >
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-slate-400">
        Dashboard / Products / {product.categoryLabel} /{" "}
      ...
      <section className="grid gap-5 xl:grid-cols-2">
            ...
            <div className="mt-3 flex gap-3">
              {images.map((img, index) => (
                <button
                  ...
                  className={`overflow-hidden rounded-xl border p-1 ${imageIndex === index ? "border-[#f59e0b]" : "border-slate-200"}`}
                >
                  <img
                    src={img}
                    alt={product.title}
        ...
        <article className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="flex gap-3">
            <button className="flex-1 rounded-xl bg-[#f59e0b] px-4 py-3 text-base font-semibold text-[#0f172a]">
              Add to Cart
            </button>
            <Link
              to="/products"
